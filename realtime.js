// realtime.js — мгновенный чат, ✓✓, «печатает…», реакции
...
function showReactionPicker(m){
  ...
  box.style.cssText='background:var(--card-solid);border-radius:26px;padding:12px 16px;display:flex;gap:12px;box-shadow:0 20px 60px rgba(0,0,0,.5);animation:pop .25s';
  ['👍','❤️','😂','','👏'].forEach(function(e){
    var b=document.createElement('button');
    b.textContent=e;
    b.style.cssText='font-size:28px;background:none;border:0;cursor:pointer;transition:transform .15s';
...
function ensureGlobalRT(){
  if(!sbOk||!sb||state._rtGlobal)return;
  state._rtGlobal=true;
  sb.channel('rt-chats-global')
    ...
    .subscribe();
  sb.channel('rt-notifs-'+state.user.id)
...
// Пузыри: галочки ✓/✓✓ + реакции + долгое нажатие
